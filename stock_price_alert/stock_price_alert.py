...
# ==================== 配置加载/保存 ====================
def load_config():
    """从JSON文件加载配置到UI全局变量（symbol保持字符串原样，不丢失前导零）"""
...
def save_config():
    """将UI全局变量保存到JSON文件（symbol作为字符串保存）"""
    price_list = []
...
class AlertSettingsApp:
    def __init__(self, root):
        ...
        self.root.geometry("850x500")  # 加宽以容纳名称列
    ...
    def create_price_tab(self):
        ...
        self.price_tree.column('name', width=150)
